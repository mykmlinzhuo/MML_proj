...
class SceneryDataset(DatasetFactory):
    def __init__(self, path, resolution, embed_dim, grid_size):
        super().__init__()
        print(f'Counting Scenery images from {path}/train and {path}/test')

        train_files = _list_image_files_recursively(os.path.join(path, 'train'))
        test_files = _list_image_files_recursively(os.path.join(path, 'test'))

        # 伪造 label（你不需要真的分类，只要 placeholder）
        train_labels = [0 for _ in train_files]
        test_labels = [0 for _ in test_files]

        self.train = ImageDataset(resolution, train_files, train_labels, embed_dim, grid_size)
        self.test = ImageDataset(resolution, test_files, test_labels, embed_dim, grid_size)
        self.resolution = resolution
